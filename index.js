...
function countEmails(str) {
  ...
  const coincidence = [...str.match(regexp)];
  const result = coincidence ? coincidence.length : 0;
  return result;
}
...
function checkRegexFlags(regex) {
  ...
  const checked = flags.includes("g", "m");
  return checked;
}
...
function checkRegexMethods(regex) {
  ...
  const haveFlags = regex.flags;
  if (haveFlags.includes("s")) {
    arr.push("dotAll");
  }
  if (haveFlags.includes("m")) {
    arr.push("multiline");
  }
  if (haveFlags.includes("y")) {
    arr.push("sticky");
  }
  return arr;
}
